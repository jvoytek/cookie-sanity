...
describe('stores/uploads', () => {
  ...
  describe('insertUpload', () => {
    it('successfully inserts upload with valid profile', async () => {
      ...
      global.useSupabaseClient = vi.fn(() => ({
        from: vi.fn(() => ({
          insert: vi.fn(() => ({
            select: vi.fn(() => ({
              single: vi.fn(() => Promise.resolve({ data: mockResponse, error: null }))
            }))
          }))
        }))
      }))
    ...
    it('throws error when profile is not found', async () => {
      ...
      await expect(newUploadsStore.insertUpload(mockJsonData)).rejects.toThrow('Profile not found')
    })
    ...
    it('handles Supabase error', async () => {
      ...
      await expect(newUploadsStore.insertUpload(mockJsonData)).rejects.toThrow('Database connection failed')
    })
  })
