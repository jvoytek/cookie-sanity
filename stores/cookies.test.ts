...
describe('stores/cookies', () => {
  ...
  beforeEach(() => {
    ...
    global.useOrdersStore = vi.fn(() => ({
      ...
      totalTransactionsByStatusAndCookie: vi.fn((status: string, type: string, abbreviation: string) => {
  ...
  describe('allCookiesWithInventoryTotals computed property', () => {
    ...
    it('preserves original cookie properties in inventory totals', () => {
      ...
      inventoryTotals.forEach(cookie => {
        const originalCookie = cookiesStore.allCookies.find(c => c.id === cookie.id)
  ...
  describe('state management', () => {
    ...
    it('maintains reactive state when cookies are updated', () => {
      ...
      const testCookie = cookiesStore.allCookiesWithInventoryTotals.find(c => c.abbreviation === 'TEST')
      expect(testCookie?.name).toBe('Test Cookie')
      expect(testCookie?.price).toBe(6.00)
    })
  })
})