...
export const useSeasonsStore = defineStore('seasons', () => {
    ...
    const fetchSeasons = async () => {
        try {
            ...
            const { data, error } = await supabaseClient.from('seasons').select(`*`).eq("profile", profileStore.currentProfile.id).order("year");
            if (error) throw error;
            allSeasons.value = data ?? [];

            if (profileStore.currentProfile.season) {
                const index = allSeasons.value.findIndex(c => c.id === profileStore.currentProfile.season);
    ...
    const getCurrentSeason = async (): Promise<Season> => {
        ...
        const getSeasonName = (season:Season | null) => {
            ...
            return season.troop_number + "-" + new Date(season.year).getFullYear();
        }
    ...
    return { fetchSeasons, allSeasons, currentSeason, settingsSelectedSeason, getCurrentSeason, getSeasonName, insertSeason, upsertSeason }
  });