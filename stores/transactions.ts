...
export const useTransactionsStore = defineStore("transactions", () => {
  ...
  const totalTransactionsByStatusAndCookie = computed(() => {
    return (status: string, transactionType: string, cookieAbbreviation: string) => {
  ...
  const sumTransactionsByCookie = computed(() => (cookieAbbreviation: string): number =>
  ...
  const _getTransactionListByStatusAndType: Ref<Order[]> = (status: string, type: "girl"|"troop"): Order[] => {
    const isTransactionType = type == "girl" ? _isGirlTransactionType : _isTroopTransactionType;
    return allTransactions.value.filter((transaction) => transaction.status === status && isTransactionType(transaction.type));
  };
  ...
  const _updateTransaction = (transaction: Order) => {
    const index = allTransactions.value.findIndex((o) => o.id === transaction.id);
    if (index !== -1) {
  ...
  const _removeTransaction = (transaction: Order | number) => {
    const transactionId: number = typeof transaction === "number" ? transaction : transaction.id;

    if (!transactionId) {
      notificationHelpers.addError({message:"Transaction ID is required to delete a transaction."} as Error);
      return;
    }

    const index = allTransactions.value.findIndex((o) => o.id === transactionId);
    if (index !== -1) {
  ...
  const _getGirlId = (name: string): number | null => {
    try {
      ...
      const matchingGirl = girlsStore.allGirls?.find(
        (girl) => girl.first_name === first_name && girl.last_name === last_name,
      );
  ...
  const _invertCookieQuantitiesInTransaction = (transaction: Order): Order[] => {
  ...
  const _supabaseInsertTransaction = async(transaction: Omit<NewOrder, "season"> & { season: number }) => {
    return await supabaseClient
      .from("orders")
      .insert(transaction)
      .select()
  ...
  const _supabaseUpdateTransactionStatus = async (transactionId: number, status: string) => {
    return await supabaseClient
    .from("orders")
  ...
  const insertNewTransaction = async (
    ...
    transaction.season = profileStore.currentProfile.season || seasonsStore.allSeasons[0].id;
  ...
  const insertNewTransactionFromUploads = async (transactionsList: NewOrder[]) => {
  ...
  const deleteTransaction = async (transaction: Order | number) => {
    ...
    if (!transactionId) {
      notificationHelpers.addError({message:"Transaction ID is required to delete a transaction."});
      return;
    }
  ...
  const convertSCOrderToNewTransaction = (obj: SCOrder2025): NewOrder | undefined => {
  ...
  const updateTransactionStatus = async (transactionId: number, status: string) => {
    try {
      const { data, error } = await _supabaseUpdateTransactionStatus(transactionId, status);
      if (error) throw error;
      data.cookies = _invertCookieQuantities(data.cookies);
      _updateTransaction(data);
      _sortTransactions();
      notificationHelpers.addSuccess(`Transaction Marked ${status.charAt(0).toUpperCase() + status.slice(1)}`);
    } catch (error) {
      notificationHelpers.addError(error as Error);
    }
  };
