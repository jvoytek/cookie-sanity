...
describe('stores/booths', () => {
  ...
  describe('computed properties', () => {
    ...
    it('filters upcoming booth sales correctly', () => {
      ...
      expect(upcoming.every(sale => sale.sale_date >= new Date().toISOString().split('T')[0])).toBe(true)
    })
  ...
  describe('fetchBoothSales', () => {
    it('successfully fetches booth sales', async () => {
      const mockBoothSales = [
        { id: 1, sale_date: '2024-01-01', sale_time: '10:00', inventory_type: 'troop' },
        { id: 2, sale_date: '2024-01-02', sale_time: '14:00', inventory_type: 'girl' }
      ]
      ...
      global.useSupabaseClient = vi.fn(() => ({
        from: vi.fn(() => ({
          select: vi.fn(() => ({
            eq: vi.fn(() => ({
              eq: vi.fn(() => ({
                order: vi.fn(() => Promise.resolve({ data: mockBoothSales, error: null }))
              }))
            }))
          }))
        }))
    ...
    it('successfully inserts booth sale with auto-calculated predictions', async () => {
      ...
      global.useSupabaseClient = vi.fn(() => ({
        from: vi.fn(() => ({
          insert: vi.fn(() => ({
            select: vi.fn(() => ({
              single: vi.fn(() => Promise.resolve({ data: mockInsertedSale, error: null }))
            }))
          }))
        }))
      }))
    ...
    it('removes auto_calculate_predicted_cookies property if present', async () => {
      ...
      global.useSupabaseClient = vi.fn(() => ({
        from: vi.fn(() => ({
          insert: vi.fn(() => ({
            select: vi.fn(() => ({
              single: vi.fn(() => Promise.resolve({ data: { id: 1, ...mockBoothSale }, error: null }))
            }))
          }))
        }))
      }))
  ...
  describe('upsertBoothSale', () => {
    beforeEach(() => {
      boothsStore.allBoothSales = [
        { id: 1, sale_date: '2024-01-01', expected_sales: 100, inventory_type: 'troop' }
      ]
    })
    ...
    it('successfully upserts booth sale and shows success toast', async () => {
      ...
      const updatedSale = { id: 1, sale_date: '2024-01-01', expected_sales: 150, inventory_type: 'troop' }
    ...
    it('handles upsert error and shows error toast', async () => {
      ...
      const updatedSale = { id: 1, sale_date: '2024-01-01', expected_sales: 150 }
      await newBoothsStore.upsertBoothSale(updatedSale)
  ...
  describe('deleteBoothSale', () => {
    ...
    it('successfully deletes booth sale and shows success toast', async () => {
      ...
      const saleToDelete = { id: 1, sale_date: '2024-01-01', inventory_type: 'troop' }
  ...
  describe('private functions integration', () => {
    it('sorts booth sales by date and time correctly', async () => {
      ...
      const mockInsertedSale = { id: 1, ...mockBoothSale, profile: 'test-user-id', season: 1 }
      ...
      boothsStore.allBoothSales = [
        { id: 2, sale_date: '2024-01-01', sale_time: '10:00', expected_sales: 100 },
        { id: 3, sale_date: '2024-01-02', sale_time: '09:00', expected_sales: 75 }
      ]
