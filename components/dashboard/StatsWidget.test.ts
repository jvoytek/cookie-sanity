...
describe("StatsWidget", () => {
  ...
  it("has consistent card structure", () => {
    ...
    cards.forEach((card) => {
      ...
      expect(card.find(".text-muted-color.font-medium.mb-4").exists()).toBe(
        true,
      );
      expect(
        card
          .find(".text-surface-900.dark\\:text-surface-0.font-medium.text-xl")
          .exists(),
      ).toBe(true);
  ...
  it("contains proper semantic structure", () => {
    ...
    const statValues = wrapper.findAll(
      ".text-surface-900.dark\\:text-surface-0.font-medium.text-xl",
    );
