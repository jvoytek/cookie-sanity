...
const { data: products } = await useAsyncData(
  'products', 
  async () => supabase
  .from('cookies').select(`*`).eq('profile', user.value.id), { transform: result => result.data })
...
async function deleteSelectedProducts() {
  try {
    const { error } = await supabase
      .from('cookies')
      .delete()
      .in('id', selectedProducts.value.map((val) => val.id))
...
async function onRowReorder(event) {
  ...
  try {
    const { error } = await supabase
      .from('cookies')
      .upsert(products.value)
    if (error) throw error
    toast.add({ severity: 'success', summary: 'Product Reordered', life: 3000 });
  } catch (error) {
    toast.add({ severity: 'error', summary: 'Error', detail: error.message, life: 3000 });
  }
              ...
              <div class="card">
                  ...
                  >
                      ...
                      <Column field="color" header="Color">
                        <template #body="slotProps">
                            <Badge size="xlarge" :style="{ backgroundColor: slotProps.data.color }" />
                        </template>
                      </Column>
                  ...
                  <div class="flex flex-col gap-6">
                      ...
                      <div class="grid grid-cols-12 gap-4">
                        ...
                        <div class="flex flex-col w-full col-span-11">
                          <InputMask id="color" v-model.trim="product.color" mask="#******" required="true" :invalid="submitted && !product.color" />
                        </div>
                        <div>
                          <small v-if="submitted && !product.color" class="text-red-500">Color is required.</small>
                        </div>
                      </div>
