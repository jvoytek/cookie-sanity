...
<template>
  <div class="col-span-12 lg:col-span-8 xl:col-span-8">
    <div class="card">
      ...
      <Select v-model="seasonsStore.settingsSelectedSeason" :options="seasonsStore.allSeasons" :option-label="seasonsStore.getSeasonName" placeholder="Select a Season" class="w-full md:w-56" @change="showSettingsForSeason" />
      <Button
        label="New"
        icon="pi pi-plus"
        severity="secondary"
          ...
          <div class="flex flex-col gap-6">
            <div>
              ...
              <InputText
                ...
                fluid
              />
              <small v-if="seasonSubmitted && !season.troop_number" class="text-red-500"
                >Troop Number is required.</small
              >
            </div>
            <div>
              <label for="name" class="block font-bold mb-3">Year</label>
              <DatePicker v-model="season.year" date-format="yy" required="true" :invalid="seasonSubmitted && !season.year" fluid/>
